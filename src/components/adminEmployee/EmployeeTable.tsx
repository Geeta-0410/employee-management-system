...
function EmployeeTable({
  ...
  const [companyCount, setCompanyCount] = useState<Record<string, number>>({});
  ...
  const defaultEmployees: Employee[] = [
    {
      employeeId: -1,
      name: "Geeta Chahar",
      email: "geetchahar0608@gmail.com",
      company: "Google",
    } as Employee,
    {
      employeeId: -2,
      name: "Admin User",
      email: "admin@codestore.com",
      company: "CODESTORE",
    } as Employee,
  ];
  ...
  useEffect(() => {
    ...
    setDomain(Object.keys(map).filter((key) => map[key] === max));
  }, [allEmployees]);
          ...
          <table className="min-w-[500px] w-full table-auto">
            ...
            <tbody className="divide-y divide-gray-200">
              {allEmployees.length > 0 ? (
                allEmployees.map((user: Employee, index: number) => {
                  const isDefaultEmployee =
                    user.employeeId === -1 || user.employeeId === -2;
                  const userDomain = user.email?.split("@")[1] ?? "";
                  const isCommonDomain = domain.includes(userDomain);
                  return (
                    <tr
                      key={user.employeeId}
                      className={
                    ...
                    >
                      ...
                      <td className="px-6 py-4 ">
                        <div className="flex justify-center items-center">
                          <button
                            disabled={isDefaultEmployee}
                            onClick={() => handleEdit(user)}
                            className={`inline-flex items-center justify-center h-10 w-10 rounded-xl border transition-all duration-200 ${
                              isDefaultEmployee
                          ...
                          <button
                            ...
                            className={`ml-3 inline-flex items-center justify-center h-10 w-10 rounded-xl border transition-all duration-200 ${
                              isDefaultEmployee
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-8  text-gray-500">
                    <p className="text-lg">
                      No users found. Add employees using the registration form
                      above.
                    </p>
                  </td>
                </tr>
  ...
  Total Employees: {totalRecords + defaultEmployees.length}
</p>
